...
def index(request):
    ...
    files_obj = student_file.objects.all()
    files_user_info_obj = User_info.objects.filter(email=student_file.objects.get('email_id'))
    ...
    file_obj_list = list(files_obj)
    #student_file_obj_list = list(student_file_obj)
    for i in (file_obj_list):
        i['project_name'] = project.objects.filter(id=int(files_user_info_obj[0].project_id))[0].project_name
        i['project_teacher'] = Teacher_info.objects.filter(Q(project_1=project.objects.filter(id=int(files_user_info_obj[0].project_id))[0].id)
                                    | Q(project_2=project.objects.filter(id=int(files_user_info_obj[0].project_id))[0].id)
                                    | Q(project_3=project.objects.filter(id=int(files_user_info_obj[0].project_id))[0].id)
                                    )[0].teacher_name
